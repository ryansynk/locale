...
#SBATCH --array=0-2
#SBATCH --job-name=queue_size_ablation
...
if [ "$SLURM_ARRAY_TASK_ID" -eq 0 ]; then
    QUEUE_SIZE="--moco_queue_size 16384"
elif [ "$SLURM_ARRAY_TASK_ID" -eq 1 ]; then
    QUEUE_SIZE="--moco_queue_size 8192"
elif [ "$SLURM_ARRAY_TASK_ID" -eq 2 ]; then
    QUEUE_SIZE="--moco_queue_size 4096"
fi
